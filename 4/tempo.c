...
void updateState(tnodo nodo, tnodo next, int nodoId, int nextId, int status, int n) {
    int lastStatus = nodo.state[nextId] % 2;
    ...
    for (int x = 0; x < n; x++) {
        if ((x == nodoId) || x == nextId) {
            continue;
        }

        if (next.state[x] > nodo.state[x]) {
            nodo.state[x] = next.state[x];
        }
    }
}
    ...
    while(time() < 500.0) {
        ...
        case TEST:
            ...
            while (status(nodo[next].id) == 1) {
                updateState(nodo[token], nodo[next], token, next, 1, n);
            ...
            if (next != token) { // para nao se auto testar
                updateState(nodo[token], nodo[next], token, next, 0, n);
